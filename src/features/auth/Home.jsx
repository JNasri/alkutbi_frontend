...
const Home = () => {
  ...
  const totalPurchaseAmountWithoutVisa = isSuccess
    ? dStat.totalPurchaseAmountWithoutVisa
    : 0;
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-8">
      {cards.map((card, index) => (
        ...
        >
          ...
          <h3
            className={`text-xl font-bold ${card.color || "text-gray-900 dark:text-white"}`}
          >
            {card.value}
          </h3>
        </div>
  ...
  return (
    <section className="p-2 animate-in fade-in duration-500">
      ...
      <div
        className={`text-2xl mb-10 text-gray-600 dark:text-gray-400 ${currentLang === "en" ? "uppercase tracking-tight" : ""}`}
      >
        👋 {t("login_welcome")}{" "}
        <span
          className={`text-gray-900 dark:text-white ${currentLang === "ar" ? "font-bold" : "font-black"}`}
        >
          {userName}
        </span>
      </div>
      ...
      <div className="mb-10">
        <div className="p-6 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-900 border-2 border-blue-200 dark:border-blue-900/50 rounded-2xl shadow-sm inline-block min-w-[320px]">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p
              className={`text-sm font-bold text-blue-600 dark:text-blue-400 ${currentLang === "en" ? "uppercase tracking-widest" : ""}`}
            >
              {t("total_balance")}
            </p>
          </div>
      ...
      <div
        className={`mb-2 text-md text-gray-800 dark:text-gray-100 ml-1 ${currentLang === "en" ? "uppercase tracking-widest" : ""}`}
      >
        {t("purchase_orders")}
      </div>
