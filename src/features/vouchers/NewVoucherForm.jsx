...
import { useAddNewVoucherMutation } from "./vouchersApiSlice";
...
const NewVoucherForm = () => {
  const [addNewVoucher, { isLoading, isSuccess, isError, error }] =
    useAddNewVoucherMutation();

  const navigate = useNavigate();
  const { t } = useTranslation();

  // useStates for all fields
  ...
  const [numberOfMovements, setNumberOfMovements] = useState(5);
  ...
  // Function to create default movements based on NumOfMovements
  const createDefaultMovements = (num) => {
    return Array.from({ length: num }, () => createMovement());
  };
  const [movements, setMovements] = useState(
    createDefaultMovements(numberOfMovements)
  );
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      toast.success(`${t("Voucher.voucherAdded")}`, {
        position: "top-center",
        type: "success",
        autoClose: 2000,
  ...
  const onNumOfMovementsChanged = (e) => {
    ...
    setMovements(createDefaultMovements(e.target.value));
  };
  ...
  const onSaveVoucherClicked = async (e) => {
    e.preventDefault();
    if (canSave) {
      // console.log("🚀 Submitting NumOfMovements:", NumOfMovements);
      // console.log("Type:", typeof NumOfMovements);
    ...
    <>
      ...
      <div className="bg-gray-100border-2  rounded-3xl shadow relative m-10">
        ...
        <div className="flex items-start justify-between p-5 border-b rounded-t">
          <h3 className="text-2xl font-semibold">{t("Voucher.addVoucher")}</h3>
        </div>
        {/* Form */}
